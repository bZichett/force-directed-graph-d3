...
export default class ForceDirectedGraph {
	...
	draw() {
		...
		node.on("dblclick.zoom", function (d) {
			var EVENT = d3.event()
			var dcx = (window.innerWidth / 2 - d.x * EVENT.transform.k);
			var dcy = (window.innerHeight / 2 - d.y * EVENT.transform.k);
			zoom.translate([dcx, dcy]);
			g.attr("transform", "translate(" + dcx + "," + dcy + ")scale(" + EVENT.transform.k + ")");
		});
